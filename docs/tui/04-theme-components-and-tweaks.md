...
styles with **semantic roles**, the reusable render primitives (notched
panel, block bar, threat dots, buttons, flash messages), and the **Tweaks**
overlay where players adjust their experience. No screen may hardcode a hex
value or draw its own border after this lands.
...
- `internal/tui/theme/` — `theme.go` (palette + roles), `panel.go`, `bar.go`,
  `widgets.go` (dots, buttons, badges, flash), tests
...
| `gold` | `#ffcf5e` | LEGENDARY tier, clean outcome |
| `red` | `#ff5a6a` | danger, raided outcome |
| `amber` | `#ffae3f` | warning, costs, stranded outcome |
| `green` | `#5fe0a0` | (reserve — confirmations) |
...
`theme.ButtonDisabled`. Screens never touch hexes.
...
- **Badge**: `▮▮ OVERDRIVE`, `☠ CONTACT IMMINENT` — small colored tags.
...
| PIRATE AGGRESSION | 0.5 / 1.0 / 1.5 / 2.0 | 1.0 | multiplier into pirate rate (gameplay/02) — self-serve difficulty |
...
| REDUCED MOTION | on / off | off | disables blink, radar sweep, overdrive pulse |
...
- [ ] `go test ./internal/tui/theme` — goldens for panel (several widths,
  ...
  thresholds, dots, buttons enabled/disabled, ASCII-safe variants.
